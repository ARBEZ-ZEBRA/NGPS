...
def render_image(
    ...
    def alpha_fn(t_starts, t_ends, ray_indices):
        ...
        dists = torch.reshape((t_ends - t_starts) / 2.0, (-1, 1))
        alphas = ngp_model.get_alpha(positions, directions)
        return alphas
    ...
    def rgb_alpha_fn(t_starts, t_ends, ray_indices):
        ...
        rgbs, alphas, normals, density = ngp_model(positions, directions)
        return rgbs, alphas    
...
def render_image_d(
    ...
    rays_o = rays_o_total.cuda()
    rays_d = rays_d_total.cuda()
    def density_fn(t_starts, t_ends, ray_indices):
        origins = rays_o[ray_indices]
        directions = rays_d[ray_indices]
        ts = torch.reshape((t_starts + t_ends) / 2.0, (-1, 1))
        positions = origins + directions * ts
...
def render_image_n(
    ...
    def normal_fn(t_starts, t_ends, ray_indices):
        origins = rays_o[ray_indices]
        directions = rays_d[ray_indices]
        ts = torch.reshape((t_starts + t_ends) / 2.0, (-1, 1))
        positions = origins + directions * ts
    ...
    normals = normal_fn(t_starts, t_ends, ray_indices)
    normal0 = nerfacc.accumulate_along_rays(weights, values=normals[:, [0]], ray_indices = ray_indices, n_rays=num_pixels)
    normal0 = torch.norm(normal0, p=2, dim=-1, keepdim=True)
    normal1 = nerfacc.accumulate_along_rays(weights, values=normals[:, [1]], ray_indices = ray_indices, n_rays=num_pixels)
    normal1 = torch.norm(normal1, p=2, dim=-1, keepdim=True)
    normal2 = nerfacc.accumulate_along_rays(weights, values=normals[:, [2]], ray_indices = ray_indices, n_rays=num_pixels)
    normal2 = torch.norm(normal2, p=2, dim=-1, keepdim=True)
    normal = torch.stack([normal0, normal1, normal2], dim=1).squeeze(-1)
